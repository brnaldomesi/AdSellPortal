<nav class="navbar navbar-default navbar-filters">
    <div class="container-fluid">
      ...
      <div class="navbar-header">
        ...
        <a class="navbar-brand" href="#">{{ trans('admin::messages.Filters') }}</a>
      </div>
        ...
        <ul class="nav navbar-nav">
          <!-- THE ACTUAL FILTERS -->
			@foreach ($xPanel->filters as $filter)
				@include($filter->view)
			@endforeach
          <li><a href="#" id="remove_filters_button"><i class="fa fa-eraser"></i> {{ trans('admin::messages.Remove filters') }}</a></li>
        </ul>
      </div><!-- /.navbar-collapse -->
    </div><!-- /.container-fluid -->
  </nav>
      ...
      jQuery(document).ready(function($) {
      	$("#remove_filters_button").click(function(e) {
		    ...
		    @else
		    	...
		    	var new_url = '{{ url($xPanel->route.'/search') }}';
